#!/usr/bin/env python3
"""
Split a seniority JSONL dataset by resume_key (no row leakage between splits).

Run from repository root:

  python ml/training/src/split_dataset.py \\
    --in ml/data/processed/seniority_from_db.jsonl \\
    --out_dir ml/data/splits/seniority_run1 \\
    --seed 42

Requires ``backend/src`` on PYTHONPATH (script adds it relative to repo root).
"""
...
def main() -> int:
    ...
    p = argparse.ArgumentParser(description="Split seniority JSONL by resume_key.")
    p.add_argument("--in", dest="in_path", required=True, help="Input JSONL (schema v1.0).")
